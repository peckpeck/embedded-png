use crate::colors::{AlphaHandler, DontDraw, PixelsIterator, ReturnC};
...
impl<'src, H> ParsedPng<'src, H> {
    pub fn from_bytes(
        ...
        let pixel_type =
            PixelType::new(header.color_type, header.bit_depth, palette, transparency)?;
        ...
        H: AlphaHandler<C>,
    {
        let mut decompressor =
            ChunkDecompressor::new_ref(self.data_chunks, buffer, self.crc_checked);
        ...
        match self.header.interlace_method {
            InterlaceMethod::None => {
                ...
                for y in 0..self.header.height {
                    ...
                    decompressor.decode_next_scanline(
                        &mut scanline_buf[..bytes_per_scanline],
                        bytes_per_pixel,
                    )?;
    ...
    where
        for<'a> F: FnMut(
            usize,
            usize,
            <PixelsIterator<'a, C, H> as Iterator>::Item,
        ) -> Result<(), DecodeError>,
        ...
        H: AlphaHandler<C>,
    {
        self.draw_inner(
            buffer,
            scanline_buf,
            |y: usize, pass: usize, it: PixelsIterator<C, H>| {
            ...
            |y: usize, pass: usize, it: PixelsIterator<C, H>| {
                if pass == 0 {
                    let line = Rectangle::new(
                        ...
                        Size::new(self.header.width as u32, 1),
                    );
                    target
                        .fill_contiguous(&line, it)
                        .map_err(|_| DecodeError::MissingBytes)
                } else {
                    target
                        .draw_iter(it.enumerate().map(|(x, c)| {
                            let (x, y) = get_xy(pass, x, y);
...
impl<'src> ParsedPng<'src, DontDraw> {
        ...
        C: PixelColor,
    {
        self.draw_inner(
            buffer,
            scanline_buf,
            |y: usize, pass: usize, it: PixelsIterator<C, DontDraw>| {
                target
                    .draw_iter(it.enumerate().filter_map(|(x, (a, c))| {
                        if a >= 128 {
                            let (x, y) = get_xy(pass, x, y);
